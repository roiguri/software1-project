...
/**
 * Performs matrix multiplication utilizing basic linear algebra to handle
 * transpositions within the multiplication process, thus avoiding the need
 * for extra space to store transposed matrices. The details of the linear algebra involved
 * are not covered here to avoid delving into the math behind.
 */
void matrix_multiply_in_place(Matrix *res, Matrix *mat1, Matrix *mat2, int transpose_mat1, int transpose_mat2)
{
    int i, j, k, m1_rows, m1_cols, m2_rows, m2_cols;
    if (mat1 == NULL || mat2 == NULL)
    {
        return;
    }
    m1_rows = transpose_mat1 ? mat1->width : mat1->length;
    m1_cols = transpose_mat1 ? mat1->length : mat1->width;
    m2_rows = transpose_mat2 ? mat2->width : mat2->length;
    m2_cols = transpose_mat2 ? mat2->length : mat2->width;

    if (res == NULL || mat1 == NULL || mat2 == NULL || m1_cols != m2_rows || res->length != m1_rows || res->width != m2_cols)
        return;
    ...
    for (i = 0; i < m1_rows; i++)
    {
        for (j = 0; j < m2_cols; j++)
        {
            ...
            for (k = 0; k < m1_cols; k++)
            {
                double m1_val = transpose_mat1 ? mat1->points[k][i] : mat1->points[i][k];
                double m2_val = transpose_mat2 ? mat2->points[j][k] : mat2->points[k][j];
                res->points[i][j] += m1_val * m2_val;
            }
        }
    }
}
